...
from app.database import PalDB
...
class RouterPal:
    def __init__(self):
        ...
        self.router.add_api_route("/work_type/{work_type}/level/{level}", self.get_pal_by_work_type_and_level_view, methods=["GET"], response_model=List[PalDB.PalView])
        self.router.add_api_route("/work_type/{work_type}/range_level/{level_min}/{level_max}", self.get_pal_by_work_type_and_level_range_view, methods=["GET"], response_model=List[PalDB.PalView])
        self.router.add_api_route("/work_type/{work_type}/min_level/{level_min}", self.get_pal_by_work_type_and_level_min_view, methods=["GET"], response_model=List[PalDB.PalView])
    ...
    @staticmethod
    async def get_pal_by_id_view(pal_id: int, db=Depends(get_db)) -> PalDB.PalView:
    ...
    async def get_pal_by_work_type_view(work_type: str, db=Depends(get_db)) -> List[PalDB.PalView]:
        pal = PalDB.get_pal_by_work_type(db, work_type)
        if pal is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pal not found")
        return pal

    @staticmethod
    async def get_pal_by_work_type_and_level_view(work_type: str, level: int, db=Depends(get_db)) -> List[PalDB.PalView]:
        pal = PalDB.get_pal_by_work_type_and_level(db, work_type, level)
        if pal is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pal not found")
        return pal
    ...
    async def get_pal_by_work_type_and_level_range_view(work_type: str, level_min: int, level_max: int, db=Depends(get_db)) -> List[PalDB.PalView]:
        pal = PalDB.get_pal_by_work_type_and_level_range(db, work_type, level_min, level_max)
        if pal is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pal not found")
        return pal

    @staticmethod
    async def get_pal_by_work_type_and_level_min_view(work_type: str, level_min: int, db=Depends(get_db)) -> List[PalDB.PalView]:
        pal = PalDB.get_pal_by_work_type_and_level_min(db, work_type, level_min)
        if pal is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pal not found")
        return pal
    ...
    @staticmethod
    async def delete_pal_view(pal_id: int, db=Depends(get_db)) -> PalDB.PalView:
        pal = PalDB.get_pal_by_id(db, pal_id)
        if pal is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Pal not found")
